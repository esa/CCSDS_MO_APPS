...
private final LocalDateTime Epoch = LocalDateTime.of(2003, Month.JUNE, 1, 0, 0);
...
public static class OrbitParameters{
    ...
    public OrbitParameters(double latitude, double longitude, double a, Vector velocity, LocalDateTime time) {
    ...
    public double getGPSaltitude(){ return a - R_e ;}
  ...
  public Orbit(double a, double i, double RAAN, double arg_per, double true_anomaly) {
    ...
    double time_int = time.toEpochSecond(ZoneOffset.UTC) - this.Epoch.toEpochSecond(ZoneOffset.UTC);
...
//    System.out.printf("RAAN: %f\nPeriod: %f\nTime int: %f\n\n", this.RAAN, Period, time_int );
  ...
  private double calculateMeanAnomaly(LocalDateTime now){
      // t: is the time since epoch
      double t = now.toEpochSecond(ZoneOffset.UTC) - this.Epoch.toEpochSecond(ZoneOffset.UTC);
      return (this.n * t);
  }
